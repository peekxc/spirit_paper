...
#lemma[
	For any $p >= 0$ and simplicial complex $K$ with $n = abs(K^p)$ and $m = abs(K^(p + 1))$, if there exists a hash function $h:K^p -> lr([n])$ constructible in $O (m)$ time supporting $O (1)$ access time and $O (c)$ space, then there exists a two-phase algorithm for computing $x |-> L_p x$ in $O (m lr((p + 1)))$ time and $O (max lr((c , m)))$ space.
] <lemma:matvec_lap>
...
Fortunately, it turns out accurately estimating eigenvalues is not necessary for estimating the rank. Recently, it has been shown that the Lanczos method is stable for matrix function#footnote[Recall that if $A in bb(R)^(n times n)$ has eigenvalue decomposition $A = U Lambda U^T$, the matrix function $f (A) in bb(R)^(n times n)$ with respect to a function $f$ is $U f (Lambda) U^T$, where $f (Lambda)$ applies $f$ to each diagonal entry of $Lambda$.] approximation even in the finite precision arithmetic model. The use of Lanczos for matrix function approximation is motivated by the fact that Lanczos expansion up to degree $k$ can _exactly_ apply any matrix polynomial $p$ with $op("degree") < k$ @musco2018stability:
...
where $T_k^((j))$ represents the Jacobi matrix of the degree-$(k + 1)$ Krylov expansion of $lr((A , v^((j))))$ and $(theta_i , y_i)$ represent the Rayleigh-Ritz pairs associated with the tridiagonal eigendecomposition $T_k = Y Theta Y^T$. When $v^((j)) tilde.op cal(D)$ derives from a sub-Gaussian distribution satisfying $bb(E) lr([v^((j)) times.circle v^((j))]) = I$, the approximation @eq:gh_trace_estimator is known to be an unbiased estimator of $tr (f (A))$. Under mild assumptions (see @ubaru2016fast), if the function of interest $f:lr([a , b]) -> bb(R)$ is analytic on $lr([lambda_min, lambda_max])$, then for constants $epsilon.alt , eta in (0 , 1)$ the GH estimator $Gamma$ satisfies: 

$ Pr(abs(tr (f (A)) - Gamma) <= epsilon.alt abs(tr (f (A)))) >= 1 - eta $ 

In other words, we can achieve a relative $epsilon.alt$-approximation of $tr (f (A))$ with success probability $eta$ using on the order of $O lr((epsilon.alt^(- 2) log lr((eta^(- 1)))))$ evaluations of $e_1^T f (T_k) e_1$ over random isotropic vectors $v tilde.op cal(D)$. 

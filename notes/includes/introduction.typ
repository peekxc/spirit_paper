...
#strong[Contributions:] Our primary contribution is the introduction of several families of spectral approximations to the rank invariants—$mu_p$ and $beta_p$—all of which are Lipshitz continuous, and differentiable on the positive semi-definite cone (@prop:operator_props). Leveraging the vast spectral theory that exists for Laplacian operators (@sec:laplacian_theory2), we show these approximations are computable in $O (m)$ memory and $O (m n)$ time, where $n$ ($m$) are the number of $p$ ($p + 1$, respectively) simplices in $K$ (@prop:spectral_rank_complexity). Moreover, both relaxations recover their corresponding invariants when $epsilon.alt$ is made small enough, and both admit iterative $lr((1 plus.minus epsilon.alt))$-approximation schemes (@sec:iterative_approx).  
